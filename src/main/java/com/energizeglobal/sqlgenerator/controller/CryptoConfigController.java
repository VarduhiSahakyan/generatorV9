...
public class CryptoConfigController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<String> updateCryptoConfig(@RequestBody CryptoConfigDTO cryptoConfigDTO, @PathVariable("id") long id) {
        String filename = cryptoConfigService.updateCryptoConfig(cryptoConfigDTO,id);
        return ResponseEntity.ok(filename);
    }
